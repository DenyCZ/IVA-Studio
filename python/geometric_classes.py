...
class BoundingBox(ProtobufTranscoder):
    def __init__(self, x, y, w, h):
        '''
                :param x: X-coordination of center of bounding box
                :param y: Y-coordination of center of bounding box
    ...
    def getProtobufEquivalent(self, dest=None, sharedStorage=None):
        if dest is None:
            dest = pb.BBox()
        dest.x1, dest.y1, dest.x2, dest.y2 = self.x, self.y, self.w, self.h
        return dest
    @classmethod
    def fromProtobufEquivalent(cls, pbmessage):
        assert isinstance(pbmessage, pb.BBox)
        return cls(pbmessage.x1, pbmessage.y1, pbmessage.x2, pbmessage.y2)
